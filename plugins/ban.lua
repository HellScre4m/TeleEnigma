...
function run(msg, matches)
    ...
    if matches[1] == "ایدی" or matches[1] == "آیدی" or matches[1]:lower() == "id" then
    ...
    -- Silent SomeOne
    if (matches[1] == "سکوت" or matches[1]:lower() == "silent") and matches[2] and not msg.reply_id and is_momod(msg) then
        if string.match(matches[2], "%d+$") then
            user_id = tonumber(matches[2])
            chat_id = msg.to.id
            hash = 'enigma:cli:mute_user:' .. chat_id
    ...
    -- silent with reply
    if (matches[1] == "سکوت" or matches[1]:lower() == "silent") and msg.reply_id and is_momod(msg) then
        local res = {
            cmd = "silent_reply",
            msg = msg
        }
    ...
    -- id with reply
    if (matches[1] == "ایدی" or matches[1] == "آیدی" or matches[1]:lower() == "id") and msg.reply_id and is_momod(msg) then
        local res = {
            cmd = "id_reply",
            msg = msg
        }
    ...
    -- idfrom with reply
    if (matches[1] == "ایدی فروارد" or matches[1] == "آیدی فروارد" or matches[1]:lower() == "idfrom") and msg.reply_id and is_momod(msg) then
        local res = {
            cmd = "idfrom_reply",
            msg = msg
        }
    ...
    -- id username
    if (matches[1] == "ایدی" or matches[1] == "آیدی" or matches[1]:lower() == "id") and matches[2] and not msg.reply_id and is_momod(msg) then
        if string.match(matches[2], "@[%a%d]") then
            local username = string.gsub(matches[2], "@", "")
            local res = {
                cmd = "id_username",
    ...
    ------------------------------------------------------
    -- mute users list
    if (matches[1] == "لیست سکوت" or matches[1] == "لیست بلاک" or matches[1]:lower() == "silentlist") and is_momod(msg) then
    ...
    -- banlist
    if (matches[1] == "لیست مسدود" or matches[1] == "لیست بلاک" or matches[1]:lower() == "banlist") and is_momod(msg) then
